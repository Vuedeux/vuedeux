...
export const STORAGE_KEY = 'todos-VUE/REDUX';

export const todoArray = JSON.parse(window.localStorage.getItem(STORAGE_KEY)) || [];


export default function todoReducer(todos = todoArray, action) {
  switch (action.type) {
    case ADD_TODO:
      return [
        {
    ...
    case COMPLETE_TODO:
      return todos.map(todo => todo.id === action.todo.id ? { ...todo, done: !todo.done } : todo);
